...
/// octiswap is the main contract for managing the swap pools and liquidity.
/// It implements the octiswap functionality.
#[near_bindgen]
...
mod tests {
    ...
    fn _init(attached_near: Balance) -> (Ctx, octiswap) {
        let mut ctx = Ctx::new(vec![], false);
        ctx.vm.attached_deposit = attached_near;
        testing_env!(ctx.vm.clone());
        let contract = octiswap::new("clp_owner".try_into().unwrap());
        return (ctx, contract);
    }
    fn init() -> (Ctx, octiswap) {
        _init(0)
    }
    fn init_with_storage_deposit() -> (Ctx, octiswap) {
    ...
    //     let (ctx, _c) = init();
    //     octiswap::new(ctx.accounts.current);
    // }
    ...
    fn check_and_create_pool(c: &mut octiswap, token: &AccountId) {
