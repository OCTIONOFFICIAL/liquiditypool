...
use octiswap::util::*;
use octiswap::{octiswapContract, PoolInfo};
use nep21_mintable::FungibleTokenContract;
use serde_json::json;
use std::convert::TryInto;

pub const octiswap_CONTRACT_ID: &str = "octiswap";

// Load in contract bytes at runtime. Current directory = closes Cargo.toml file location
near_sdk_sim::lazy_static_include::lazy_static_include_bytes! {
    CLP_WASM_BYTES => "../res/octiswap.wasm"
}

// Deploy octiswap Contract
pub fn deploy_clp() -> (
    UserAccount,
    ContractAccount<octiswapContract>,
    UserAccount,
    UserAccount,
    UserAccount,
) {
    ...
    let contract_user = deploy!(
        // Contract Proxy
        contract: octiswapContract,
        // Contract account id
        contract_id: octiswap_CONTRACT_ID,
        // Bytes of contract
...
pub fn get_pool_info(clp: &ContractAccount<octiswapContract>, token: &AccountId) -> PoolInfo {
...
pub fn create_pool_add_liquidity(
    clp: &ContractAccount<octiswapContract>,
    ...
    call!(
        owner,
        token_contract.inc_allowance(octiswap_CONTRACT_ID.to_string(), token_amount.into()),
