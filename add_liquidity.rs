...
fn add_liquidity() {
    ...
    let res = call!(
        alice,
        token_contract.inc_allowance(octiswap_CONTRACT_ID.to_string(), token_deposit.into()),
    ...
    let val = view!(token_contract.get_allowance(
        alice.account_id(), octiswap_CONTRACT_ID.to_string())
    );
    ...
    // Verify Liquidity
    let bal = get_nep21_balance(&token_contract, &octiswap_CONTRACT_ID.to_string());
